...
module.exports = function(cfg) {
  ...
  this.maple = {
    ...
    getTraceTreeSequenceNumber: function(req, res) {
      var opts = {
        url: cfg.endpoint +
          "/restconf/operational/maple-tracetree-history:tracetreehistory/history-count/",
        headers: {'Authorization': 'Bearer ' + this.oauth.getToken()}
      };
      request.get(opts, function(error, response, body) {
        if (error) {
        ...
        if (response.statusCode == 200 ||
            response.statusCode == 201) {
          var data = JSON.parse(body);
          var seq_num = data['history-count']['count'];
          var out = {}
          out.seq_num = seq_num;
    ...
    getPacketList: function(req, res) {
      var opts = {
        url: cfg.endpoint + 
          "/restconf/operational/maple-tracetree-history:tracetreehistory/",
        headers: {'Authorization': 'Bearer ' + this.oauth.getToken()}
      };
      request.get(opts, function(error, response, body) {
        if (error) {
        ...
        if (response.statusCode == 200 ||
          ...
          var data = JSON.parse(body);
          var packet_list = [];
          for (i in data['tracetreehistory']['history']) {
            packet_list[parseInt(data["tracetreehistory"]["history"][i]["seq"])] = {
              "timestamp": data['tracetreehistory']['history'][i]['pkt']['timestamp'],
              "packetJSON": JSON.parse(data['tracetreehistory']['history'][i]['pkt']['pkt-json-str'])
            };
          }
          var out = {'packet_list': packet_list};
          res.status(200).json(out);
          return;
        }
        res.status(400).json({error: "Unknown error"});
    ...
    getTraceTreeHistory: function(req, res) {
      var seq = parseInt(req.params["seqnum"]);
      var opts = {
        url: cfg.endpoint +
          "/restconf/operational/maple-tracetree-history:tracetreehistory/history/" + seq + "/",
        headers: {'Authorization': 'Bearer ' + this.oauth.getToken()}
      };
      request.get(opts, function(error, response, body) {
        if (error) {
        ...
        if (response.statusCode == 200 ||
            response.statusCode == 201) {
          var data = JSON.parse(body);
          var yangTT = data['history'][0]['tracetree-v2'];
          var tree = {};
          tree.ttnodes = yangTT['tt-node-v2'].map(function(n) {
            var outnode = {};
            Object.keys(n).forEach(function(k) {
              switch(k) {
                case 'action-type': // make history API like live API
                  outnode["maple-l-type:action-type"] = n[k]
                  break;
                case "matchfield":
                  outnode["maple-v-type:field"] = n[k];
                  break;
                case "link":
                  outnode["maple-l-type:link"] = n[k];
                case "testfield":
                  outnode["maple-t-type:field"] = n[k];
                default:
                  outnode[k] = n[k];
                  break;
              }
          ...
          tree.ttlinks = yangTT['tt-link-v2'].map(function(l) {
            var outlink = {};
          ...
          var packetStr = data['history'][0]['pkt']['pkt-str'];
          var packetJSON = JSON.parse(data['history'][0]['pkt']['pkt-json-str']);
          var packetTimestamp = data['history'][0]['pkt']['timestamp'];
          var out = {'tracetree': tree, 'packetStr': packetStr, 'packetJSON': packetJSON, 'timestamp': packetTimestamp};
          res.status(200).json(out);
          return;
        }
        res.status(400).json({error: "Unknown error"});
