      ...
      topology.node.forEach(function(n, i) {
        ...
        if (_this.extractNodeType(n["node-id"]) == "host") {
          ...
          node = {
            ...
            mac: _this.extractNodeNo(n["node-id"]),
            type: 'host'
          };
        } else {
          node = {
      ...
      topology.link.forEach(function(l) {
        ...
        if (!intermediates.downlinks[l["link-id"]] && !seenPort[l.source["source-tp"]] && !seenPort[l.destination["dest-tp"]]) {
          var link = {
            ...
            sourcePort: _this.extractPort(l.source["source-tp"]),
            targetPort: _this.extractPort(l.destination["dest-tp"])
          };
...
module.exports = function(cfg) {
  ...
  this.maple = {
    ...
    getTraceTree: function(req, res) {
      ...
      //     var out = {};
      //     out.ttnodes = yangTT.ttnodes.map(function(n) {
      //       var outnode = {};
      ...
      //       return outnode;
      //     });
      //     out.ttlinks = data.tracetree.ttlinks.map(function(l) {
      //       var outlink = {};
      ...
      //   }
      //   rs.status(400).json({error: "Unknown error"});
      // });
    },
        ...
        if (response.statusCode == 200 ||
          ...
          request.get(invOpts, function(error, response, body) {
            if (response.statusCode == 200 ||
              ...
              var out = utils.transform(topo, inv);
              res.status(200).json(out);
              return;
            }
