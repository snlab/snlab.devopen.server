...
module.exports = function(cfg) {
  ...
  this.maple = {
    ...
    getTraceTreeHistory: function(req, res) {
      ...
      var seq = parseInt(path.substring(n + 1));
      var opts = {
        url: cfg.endpoint +
          "/restconf/operationl/maple-tracetree-history:tracetreehistory/history/" + seq + "/",
        headers: {'Authorization': 'Bearer ' + this.oauth.getToken()}
      };
      request.get(opts, function(error, response, body) {
        if (error) {
        ...
        if (response.statusCode == 200 ||
            response.statusCode == 201) {
          var data = JSON.parse(body);
          var yangTT = data['history']['tracetree-v2'];
          var out = {};
          out.ttnodes = yangTT.tt-node-v2.map(function(n) {
            var outnode = {};
          ...
          out.ttlinks = data.tracetree.tt-link-v2.map(function(l) {
            var outlink = {};
