...
var utils = {
  ...
  transform: function(topo, inv, ht) {
    ...
    intermediates.addresses = {};
    intermediates.names = {};
    intermediates.tables = {};
    if (inv.nodes.node) {
      inv.nodes.node.forEach(function(node) {
        intermediates.addresses[node.id] = node['flow-node-inventory:ip-address'];
        intermediates.names[node.id] = node['flow-node-inventory:description'];
        intermediates.tables[node.id] = _this.getFlowTableById(node['flow-node-inventory:table'], 0);
        node["node-connector"].forEach(function(connector) {
          if (connector.id.indexOf("LOCAL") >= 0) {
            intermediates.addresses[node.id] = connector["flow-node-inventory:hardware-address"];
            intermediates.names[node.id] = connector["flow-node-inventory:name"];
          } else {
            intermediates.addresses[connector.id] = connector["flow-node-inventory:hardware-address"];
          }
        });
      });
    }
      ...
      topology.node.forEach(function(n, i) {
        ...
        if (_this.extractNodeType(n["node-id"]) == "host") {
          ...
          node = {
            deviceId: n["node-id"],
            name: intermediates.names[n["node-id"]],
            address: intermediates.addresses[n["node-id"]],
